...
function initializeEventListeners() {
    ...
    document.getElementById('augmentedFile').addEventListener('change', handleAugmentedFile);
...
async function analyzeDatasets() {
    try {
        ...
        let origPath, augPath;

        // Upload files if not loaded from DB
        if (uploadedFiles.original && uploadedFiles.augmented) {
            const formData = new FormData();
            formData.append('original', uploadedFiles.original);
            formData.append('augmented', uploadedFiles.augmented);
            ...
            augPath = uploadData.augmented_path;
        } else {
            origPath = uploadedPaths.original;
            augPath = uploadedPaths.augmented;
        }
        ...
        console.log('ANALYZE RESPONSE - results:', analysisResults);
        console.log('ANALYZE RESPONSE - summaries:', analysisResults ? analysisResults.summaries : null);
...
async function trainModels() {
    ...
    try {
        ...
        const modelConfigs = selectedModels.map(model => {
            ...
            return { type: model, params: params[model] || {} };*/
            return {
                type: model,
        ...
        // Ensure we have real paths. If files were chosen locally, upload them first.
        let origPath = uploadedPaths.original || null;
        let augPath = uploadedPaths.augmented || null;

        if ((!origPath && uploadedFiles.original) || (!augPath && uploadedFiles.augmented)) {
            const formData = new FormData();
            if (uploadedFiles.original) formData.append('original', uploadedFiles.original);
            if (uploadedFiles.augmented) formData.append('augmented', uploadedFiles.augmented);
            ...
            if (augPath) uploadedPaths.augmented = augPath;
        }
        ...
        console.log('UTILITY REQUEST', { original_path: origPath, augmented_path: augPath, target_column: targetColumn, model_configs: modelConfigs });
...
async function generateSuggestions() {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/suggestions/generate`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                model: document.getElementById('ollamaModel').value || 'llama3', // selectedModel might be empty
        ...
        if (data.success) {
            // Backend now saves to DB. 
            // If backend also returns the suggestions text in 'response' or 'suggestions', display it.
            // As per app.py: suggestions_text = response['message']['content'] ... then parsed.
            // But the endpoint only returns suggestions if analysis_id is present and they are saved?
            // Actually app.py doesn't return the text in the JSON if it just saves them?
            // Let's assume we need to reload or just show success. 
            // Wait, looking at app.py replacement: it returns suggestions if parse_suggestions works?
            // Actually, I didn't see the return statement in my app.py replacement block... 
            // Let's assume standard behavior or just alert success.
            // For now, let's display a message.

            // To be safe/robust, let's try to display what we can.
            if (data.suggestions) {
                displaySuggestions(data.suggestions); // If backend returns list
            } else {
                alert('Suggestions generated and saved to history!');
...
async function explainMetrics(category) {
    ...
    const metrics = analysisResults[category];
...
async function sendExplanationRequest(prompt) {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/explain`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                model: document.getElementById('ollamaModel').value || 'llama3',
                prompt: prompt,
                metrics: analysisResults,
                // Dual-source params
                use_cloud: useCloud,
        ...
        if (data.success) {
            ...
            modelSpan.textContent = `Model: ${data.model}`;
            if (useCloud) modelSpan.textContent += ' (Cloud)';
